  ...
  <div class="gp g-items gp-profile">
    ...
    <ul>
      ...
      <li class="link" v-if="!$route.query.realname">
        <label>昵称</label>
        <span class="right" v-if="!$route.query.realname" @click="intoacctsetinfo">{{_accountinfo.nickname}}
          <img src="@/assets/images/icon_more2@2x.png" alt="">
        </span>
      </li>
      <li>
        <label>用户名</label>
        <span class="right">{{_accountinfo.name}}</span>
      </li>
    </ul>
    <ul>
      <li class="link">
...
export default {
  ...
  methods: {
    intoacctsetinfo(e) {
      this.$router.push({
        name: 'acctestinfo',
      query: { name: this._accountinfo.nickname}
      });
    },
    changeIMG(e) {
