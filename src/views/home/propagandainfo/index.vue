    ...
    <div class="margin55" v-show="!loadingtrue">
      <div class="outpatien-card">
        <md-field>
          ...
          <md-detail-item v-else-if="feeDetailData.feeType==3||feeDetailData.feeType==4||feeDetailData.feeType==5" title="开单医生" :content=feeDetailData.doctor></md-detail-item>
        ...
        <div v-if="feeDetailData.bizDetail">
          <div v-for="(item,i) in feeDetailData.bizDetail.classList" :key="i">
            ...
            <div class="spanWid cardTextPP arrow" v-for="(item2,index) in item.list" :key="index">
              ...
              <span>{{item2.total}}{{item2.packUnit}}</span>
...
let fee_detail_url = "/api/hos/app/bizPayOrder/read/detail";
let fconfirm_pay_url = "/api/hos/app/bizCostBill/confirmPay";
let now_pay_url = "/api/hos/app/bizPayOrder/nowPay";
...
export default {
  ...
  methods: {
    feeDetail() {
      ...
      aa.feeType = this.$route.query.feeType*1;
      aa.objId = this.$route.query.objId*1;
    ...
    onCashierPay(item) {
      ...
      nowPayParams.feeType = this.$route.query.objId ? this.$route.query.feeType * 1 : null;
      if (this.click) {
        return
      } this.click = true;
